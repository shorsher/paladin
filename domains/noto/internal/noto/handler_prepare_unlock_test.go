...
func TestPrepareUnlock(t *testing.T) {
	...
	require.Len(t, assembleRes.AssembledTransaction.InfoStates, 7) // manifest + spend-unlock-manifest + unlock-data-info + prepare-data-info + cancel-unlock-manifest + output-coin + cancel-coin
	...
	cancelUnlockManifestState := assembleRes.AssembledTransaction.InfoStates[4]
	...
	expectedSpendHash, err := n.unlockHashFromIDs_V1(ctx, ethtypes.MustNewAddress(contractAddress), lockID, notoOptions.SpendTxId.HexString(), endorsableStateIDs(readStates), endorsableStateIDs(infoStates[1:2]), spendUnlockTxData)
	require.NoError(t, err)
	require.Equal(t, expectedSpendHash, fnParams.Params.SpendHash)
	expectedCancelHash, err := n.unlockHashFromIDs_V1(ctx, ethtypes.MustNewAddress(contractAddress), lockID, notoOptions.SpendTxId.HexString(), endorsableStateIDs(readStates), endorsableStateIDs(infoStates[2:3]), cancelUnlockTxData)
	require.NoError(t, err)
