...
func TestPrepareMintUnlock(t *testing.T) {
	...
	require.Len(t, assembleRes.AssembledTransaction.InfoStates, 6) // manifest + spend-unlock-manifest + cancel-unlock-manifest + unlock-data-info + prepare-data-info + output-coin
	...
	expectedSpendHash, err := n.unlockHashFromIDs_V1(ctx, ethtypes.MustNewAddress(contractAddress), lockID, notoOptions.SpendTxId.HexString(), []string{}, endorsableStateIDs(infoStates[1:2]), spendUnlockTxData)
	require.NoError(t, err)
